...
export const mentorRouter = createRouter({
  chat: authedQuery
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (lowerMsg.includes("hello") || lowerMsg.includes("hi ") || lowerMsg === "hi") {
        ...
        response = "React is a component-based UI library. Core concepts: JSX, props, state (useState), effects (useEffect), hooks, and context. The React course on Bitxy covers all of this! What specifically about React?";
  ...
  clearHistory: authedQuery.mutation(async ({ ctx }) => {
    const db = getDb();
    await db.from("chatMessages").delete().eq("userId", ctx.user.id);
    return { success: true };
  }),
});
